DATA ="""05 ANTIOQUIA 001 MEDELLIN
...
def obtenerDepartamentos():
    departamentos = []
    municipios = []
    for departamento in DATA.split("\n"):
        datos = departamento.split(" ")
        DEPARTMENTCODE = datos[0]

        NOMBRE = ''

        START_TO_MUNICIPIO = -1

        for i in range(1, len(datos)):
            if datos[i].isdigit():
                START_TO_MUNICIPIO = i
                break
            NOMBRE += f' {datos[i]}'

        if START_TO_MUNICIPIO != -1:
            for i in range(1, len(datos)):
                NOMBRE_MUNICIPIO = ''

                for i in range(START_TO_MUNICIPIO + 1, len(datos)):
                    if datos[i].isdigit():
                        break
                    NOMBRE_MUNICIPIO += f' {datos[i]}'

                municipios.append({
                    'codigo': f'{datos[START_TO_MUNICIPIO]}',
                    'descripcion': NOMBRE_MUNICIPIO,
                    'departamento_id': DEPARTMENTCODE
                })
...
MUNICIPIOS = obtenerDepartamentos()

# filtrar para que no se repita el Codigo

def filtrarMunicipios():
    municipios = []
    codigos = []
    for municipio in MUNICIPIOS:
        if municipio['codigo'] not in codigos:
            codigos.append(municipio['codigo'])
            municipios.append(municipio)
    return municipios

MUNICIPIOS = filtrarMunicipios()

print(json.dumps(MUNICIPIOS, indent=2))